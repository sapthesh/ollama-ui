...
import { openScreenshot, url, model } from './helpers';
...
export const GITHUB_ACTIONS = process.env.GITHUB_ACTIONS === 'true' || false;
...
export class AppTest {
  ...
  async close(title) {
    ...
    if (path) {
      console.log(`  ✔ Video ${path}: ${title}`);
      if (title === 'Send message') {
        console.log(`  ✔ Updated demo`);
        const destinationPath = 'videos/video.webm';
        await fs.copyFile(path, destinationPath);
        console.log(`  ✔ ${title} video copied to ${destinationPath}`);
      }
    }
  }
